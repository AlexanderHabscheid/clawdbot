...
class TrayManager {
  ...
  async showControlPanelFromTray() {
    try {
      ...
      logger.error("No control panel callback available");
    } catch (error) {
      logger.error("Failed to open control panel:", error);
    }
  }
  ...
  async createTray() {
    ...
    try {
      ...
      logger.error("Error creating tray icon:", error.message);
    }
  }
      ...
      try {
        if (fs.existsSync(testPath)) {
          ...
          if (icon && !icon.isEmpty()) {
            ...
            logger.log("Using tray icon:", testPath);
            return icon;
          }
        }
      } catch (error) {
        logger.error("Error checking tray icon path:", testPath, error.message);
      }
    }

    logger.error("Could not find tray icon in any expected location");
  ...
  createFallbackIcon() {
    try {
      ...
      logger.warn("Canvas not available, creating minimal fallback icon");
      ...
      logger.log("Created minimal fallback tray icon");
      return fallbackIcon;
    }
  }
  ...
  buildContextMenuTemplate() {
    ...
    return [
      ...
      { type: "separator" },
      {
        label: "Quit Centris",
        click: () => {
          logger.log("Quitting app via tray menu");
          app.quit();
        },
      },
    ];
  ...
  setupTrayEventHandlers() {
    ...
    this.tray.on("destroyed", () => {
      logger.log("Tray icon destroyed");
      this.tray = null;
    });
  }
}
