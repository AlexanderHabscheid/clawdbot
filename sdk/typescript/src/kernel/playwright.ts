...
interface PlaywrightPage {
  ...
  getByText(text: string): {
    first(): { isVisible(): Promise<boolean>; catch(fn: () => boolean): Promise<boolean> };
  };
...
export class PlaywrightActionKernel implements ActionKernel {
  ...
  constructor(private readonly options?: { headless?: boolean; slowMo?: number }) {}
  ...
  async setup(): Promise<void> {
    ...
    const mod = await import(/* @vite-ignore */ "playwright");
  ...
  async act(request: KernelActRequest): Promise<KernelActResult> {
    ...
    switch (request.kind) {
      ...
      default:
        return { ok: false, details: { error: `unsupported action: ${String(request.kind)}` } };
    }
  }
  ...
  async route(request: KernelRouteRequest): Promise<KernelRouteResult> {
    ...
    for (const step of request.steps) {
      if ("navigate" in step) {
        await this.act({ kind: "navigate", value: applyTemplate(step.navigate, request.params) });
        executed++;
      } else if ("click" in step) {
        await this.act({ kind: "click", target: applyTemplate(step.click, request.params) });
        executed++;
      } else if ("type" in step) {
        await this.act({
          kind: "type",
          target: applyTemplate(step.type.target, request.params),
        ...
        await this.act({ kind: "scroll", value: step.scroll, amount: step.amount });
        executed++;
      }
    }
