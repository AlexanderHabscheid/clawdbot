  ...
  if (
    ...
    this.log = function () {
      if ("undefined" != typeof console) {
        Array.from(arguments, (t) => (t && t.nodeType == this.ELEMENT_NODE ? e(t) : t)).unshift(
          "Reader: (Readability)",
        );
...
((Readability.prototype = {
      ...
      if (t) {
        if (0 === t.indexOf("javascript:"))
          if (1 === e.childNodes.length && e.childNodes[0].nodeType === this.TEXT_NODE) {
            ...
            for (var r = this._doc.createElement("span"); e.firstChild; )
  ...
  _getArticleTitle() {
    ...
    if (new RegExp(`\\s[${s}]\\s`).test(t)) {
      ...
      r((t = i.substring(0, e.pop().index))) < 3 &&
        (t = i.replace(new RegExp(`^[^${s}]*[${s}]`, "gi"), ""));
    } else if (t.includes(": ")) {
      ...
      this._someNode(n, function (e) {
        ...
        (r((t = i.substring(i.lastIndexOf(":") + 1))) < 3
          ? (t = i.substring(i.indexOf(":") + 1))
          : r(i.substr(0, i.indexOf(":"))) > 5 && (t = i));
    ...
    return (
      h <= 4 && (!a || h != r(i.replace(new RegExp(`\\s[${s}]\\s`, "g"), "")) - 1) && (t = i), t
    );
  },
  _prepDocument() {
    var e = this._doc;
  ...
  _prepArticle(e) {
    ...
    (this._forEachNode(e.children, function (e) {
      ...
      this._removeNodes(this._getAllNodesWithTag(e, ["p"]), function (e) {
        return (
          0 === this._getAllNodesWithTag(e, ["img", "embed", "object", "iframe"]).length &&
  ...
  _textSimilarity(e, t) {
    ...
    return i.length && a.length
      ? 1 - a.filter((e) => !i.includes(e)).join(" ").length / a.join(" ").length
      : 0;
  },
  _isValidByline(e, t) {
    ...
    return (
      ("author" === i || (a && a.includes("author")) || this.REGEXPS.byline.test(t)) &&
      !!r &&
      r < 100
    );
  },
  _getNodeAncestors(e, t) {
    t = t || 0;
    for (var i = 0, a = []; e.parentNode && (a.push(e.parentNode), !t || ++i !== t); ) {
      e = e.parentNode;
    }
    return a;
  },
  _grabArticle(e) {
    ...
    for (var a = e.innerHTML; ; ) {
        ...
        if (this._isProbablyVisible(n)) {
          if ("true" != n.getAttribute("aria-modal") || "dialog" != n.getAttribute("role"))
            if (this._articleByline || this._metadata.byline || !this._isValidByline(n, l))
              if (J && this._headerDuplicatesTitle(n))
                (this.log("Removing header: ", n.textContent.trim(), this._articleTitle.trim()),
                  (J = !1),
                  (n = this._removeAndGetNext(n)));
              else {
                if (r) {
                  if (
                    this.REGEXPS.unlikelyCandidates.test(l) &&
                    !this.REGEXPS.okMaybeItsACandidate.test(l) &&
                    !this._hasAncestorTag(n, "table") &&
                    !this._hasAncestorTag(n, "code") &&
                    "BODY" !== n.tagName &&
                    "A" !== n.tagName
                  ) {
                    (this.log("Removing unlikely candidate - " + l),
                      (n = this._removeAndGetNext(n)));
                    continue;
                  }
                  if (this.UNLIKELY_ROLES.includes(n.getAttribute("role"))) {
                    (this.log("Removing content with role " + n.getAttribute("role") + " - " + l),
                      (n = this._removeAndGetNext(n)));
                    continue;
                  }
                }
                if (
                  ("DIV" !== n.tagName &&
                    ...
                    "H5" !== n.tagName &&
                    "H6" !== n.tagName) ||
                  !this._isElementWithoutContent(n)
                ) {
                  if (
                    (this.DEFAULT_TAGS_TO_SCORE.includes(n.tagName) && s.push(n),
                    "DIV" === n.tagName)
                  ) {
                    for (var o = null, h = n.firstChild; h; ) {
                      var d = h.nextSibling;
                      if (this._isPhrasingContent(h))
                        null !== o
                          ? o.appendChild(h)
                          : this._isWhitespace(h) ||
                            ((o = t.createElement("p")), n.replaceChild(o, h), o.appendChild(h));
                      else if (null !== o) {
                    ...
                    if (this._hasSingleTagInsideElement(n, "P") && this._getLinkDensity(n) < 0.25) {
                      var c = n.children[0];
                      (n.parentNode.replaceChild(c, n), (n = c), s.push(n));
                    } else
                      this._hasChildBlockElement(n) || ((n = this._setNodeTag(n, "P")), s.push(n));
                  }
              ...
              for (
                var g = this._getNextNode(n, !0), _ = this._getNextNode(n), m = null;
                _ && _ != g;
              ) {
              ...
              ((this._articleByline = (m ?? n).textContent.trim()),
                (n = this._removeAndGetNext(n)));
            }
          else n = this._removeAndGetNext(n);
        } else {
          (this.log("Removing hidden node - " + l), (n = this._removeAndGetNext(n)));
        }
      }
      var p = [];
      this._forEachNode(s, function (e) {
        if (e.parentNode && void 0 !== e.parentNode.tagName) {
          ...
          if (!(t.length < 25)) {
            ...
            if (0 !== i.length) {
              var a = 0;
              ((a += 1),
                (a += t.split(this.REGEXPS.commas).length),
                (a += Math.min(Math.floor(t.length / 100), 3)),
                this._forEachNode(i, function (e, t) {
                  if (e.tagName && e.parentNode && void 0 !== e.parentNode.tagName) {
                    if (
                      (void 0 === e.readability && (this._initializeNode(e), p.push(e)), 0 === t)
                    ) {
                      var i = 1;
                    } else {
                      i = 1 === t ? 2 : 3 * t;
      ...
      if (null === S || "BODY" === S.tagName) {
        for (S = t.createElement("DIV"), C = !0; e.firstChild; ) {
          (this.log("Moving child out:", e.firstChild), S.appendChild(e.firstChild));
        }
        (e.appendChild(S), this._initializeNode(S));
      } else if (S) {
        for (var L = [], x = 1; x < N.length; x++) {
          N[x].readability.contentScore / S.readability.contentScore >= 0.75 &&
            L.push(this._getNodeAncestors(N[x]));
        }
        if (L.length >= 3) {
          for (v = S.parentNode; "BODY" !== v.tagName; ) {
        ...
        for (var R = S.readability.contentScore, P = R / 3; "BODY" !== v.tagName; ) {
          if (v.readability) {
        ...
        for (v = S.parentNode; "BODY" != v.tagName && 1 == v.children.length; ) {
          v = (S = v).parentNode;
        }
      ...
      for (
        var B = Math.max(10, 0.2 * S.readability.contentScore),
          G = (v = S.parentNode).children,
          M = 0,
          H = G.length;
        M < H;
        M++
        ...
        if (
          (this.log(
            ...
            k,
            k.readability ? "with score " + k.readability.contentScore : "",
          ),
          this.log("Sibling has score", k.readability ? k.readability.contentScore : "Unknown"),
          k === S)
        ) {
          U = !0;
        } else {
          var W = 0;
          if (
            (k.className === S.className &&
              "" !== S.className &&
              (W += 0.2 * S.readability.contentScore),
            k.readability && k.readability.contentScore + W >= B)
          ) {
            U = !0;
            ...
            var $ = this._getLinkDensity(k),
              ...
              X = F.length;
            ((X > 80 && $ < 0.25) || (X < 80 && X > 0 && 0 === $ && -1 !== F.search(/\.( |$)/))) &&
              (U = !0);
          }
        }
        U &&
          (this.log("Appending node:", k),
          this.ALTER_TO_DIV_EXCEPTIONS.includes(k.nodeName) ||
            (this.log("Altering sibling:", k, "to div."), (k = this._setNodeTag(k, "DIV"))),
          O.appendChild(k),
          (G = v.children),
          (M -= 1),
          (H -= 1));
      ...
      if (
        (this._debug && this.log("Article content pre-prep: " + O.innerHTML),
        this._prepArticle(O),
        this._debug && this.log("Article content post-prep: " + O.innerHTML),
        C)
      ) {
        ((S.id = "readability-page-1"), (S.className = "page"));
      } else {
        var j = t.createElement("DIV");
        for (j.id = "readability-page-1", j.className = "page"; O.firstChild; ) {
          j.appendChild(O.firstChild);
        }
        O.appendChild(j);
      }
      ...
      if (Y < this._charThreshold) {
        if (
          ((V = !1),
          (e.innerHTML = a),
          this._attempts.push({ articleContent: O, textLength: Y }),
  ...
  _unescapeHtmlEntities(e) {
    ...
    return e
      .replace(/&(quot|amp|apos|lt|gt);/g, function (e, i) {
        return t[i];
      })
      .replace(/&#(?:x([0-9a-f]+)|([0-9]+));/gi, function (e, t, i) {
        var a = parseInt(t || i, t ? 16 : 10);
        return (
          (0 == a || a > 1114111 || (a >= 55296 && a <= 57343)) && (a = 65533),
  ...
  _getJSONLD(e) {
    var t,
      i = this._getAllNodesWithTag(e, ["script"]);
    return (
      this._forEachNode(i, function (e) {
        if (!t && "application/ld+json" === e.getAttribute("type")) {
          try {
            var i = e.textContent.replace(/^\s*<!\[CDATA\[|\]\]>\s*$/g, ""),
              a = JSON.parse(i);
            if (
              Array.isArray(a) &&
              !(a = a.find((e) => e["@type"] && e["@type"].match(this.REGEXPS.jsonLdArticleTypes)))
            )
              return;
            var r = /^https?\:\/\/schema\.org\/?$/;
            if (
              !(
                ("string" == typeof a["@context"] && a["@context"].match(r)) ||
                ("object" == typeof a["@context"] &&
                  "string" == typeof a["@context"]["@vocab"] &&
                  a["@context"]["@vocab"].match(r))
              )
            )
              return;
            if (
              ...
              !a || !a["@type"] || !a["@type"].match(this.REGEXPS.jsonLdArticleTypes))
            )
              return;
            if (
              ((t = {}),
              "string" == typeof a.name && "string" == typeof a.headline && a.name !== a.headline)
            ) {
              var s = this._getArticleTitle(),
                n = this._textSimilarity(a.name, s) > 0.75,
                l = this._textSimilarity(a.headline, s) > 0.75;
              t.title = l && !n ? a.headline : a.name;
            } else
              "string" == typeof a.name
                ? (t.title = a.name.trim())
                : "string" == typeof a.headline && (t.title = a.headline.trim());
            (a.author &&
              ("string" == typeof a.author.name
                ? (t.byline = a.author.name.trim())
                : Array.isArray(a.author) &&
                  a.author[0] &&
                  "string" == typeof a.author[0].name &&
                  (t.byline = a.author
                    .filter(function (e) {
                    ...
                    .join(", "))),
              "string" == typeof a.description && (t.excerpt = a.description.trim()),
              a.publisher &&
                "string" == typeof a.publisher.name &&
                (t.siteName = a.publisher.name.trim()),
              "string" == typeof a.datePublished && (t.datePublished = a.datePublished.trim()));
          } catch (e) {
            this.log(e.message);
          }
        }
  ...
  _getArticleMetadata(e) {
    ...
    (this._forEachNode(a, function (e) {
      ...
      if (n) {
        var l = null,
          o = null;
        (a && (l = a.match(r)) && ((o = l[0].toLowerCase().replace(/\s/g, "")), (i[o] = n.trim())),
          !l &&
            t &&
            s.test(t) &&
            ((o = t),
            n &&
              ((o = o.toLowerCase().replace(/\s/g, "").replace(/\./g, ":")), (i[o] = n.trim()))));
      }
    }),
      (t.title =
        e.title ||
    ...
    const n =
      "string" != typeof i["article:author"] || this._isUrl(i["article:author"])
        ? void 0
        : i["article:author"];
    return (
      (t.byline =
        e.byline || i["dc:creator"] || i["dcterm:creator"] || i.author || i["parsely-author"] || n),
      (t.excerpt =
        e.excerpt ||
        i["dc:description"] ||
  ...
  _isSingleImage(e) {
    for (; e; ) {
      ...
      if (1 !== e.children.length || "" !== e.textContent.trim()) {
        return !1;
      }
      e = e.children[0];
    }
  ...
  _unwrapNoscriptImages(e) {
    ...
    this._forEachNode(t, function (e) {
      for (var t = 0; t < e.attributes.length; t++) {
        ...
        if (/\.(jpg|jpeg|png|webp)/i.test(i.value)) {
          return;
        }
      }
      e.remove();
      ...
      if (this._isSingleImage(t)) {
        ...
        if (a && this._isSingleImage(a)) {
          ...
          for (var s = i.getElementsByTagName("img")[0], n = 0; n < r.attributes.length; n++) {
            ...
            if (
              "" !== l.value &&
              ("src" === l.name || "srcset" === l.name || /\.(jpg|jpeg|png|webp)/i.test(l.value))
            ) {
              if (s.getAttribute(l.name) === l.value) {
                continue;
              }
              var o = l.name;
  ...
  _getLinkDensity(e) {
    ...
    return (
      this._forEachNode(e.getElementsByTagName("a"), function (e) {
        var t = e.getAttribute("href"),
          a = t && this.REGEXPS.hashUrl.test(t) ? 0.3 : 1;
  ...
  _hasAncestorTag(e, t, i, a) {
    ...
    for (var r = 0; e.parentNode; ) {
      ...
      if (e.parentNode.tagName === t && (!a || a(e.parentNode))) {
        return !0;
      }
      ((e = e.parentNode), r++);
    }
          ...
          if (a.getAttribute("summary")) a._readabilityDataTable = !0;
            ...
            if (r && r.childNodes.length) a._readabilityDataTable = !0;
              ...
              if (
                ...
                1 != s.columns && 1 != s.rows
                  ? s.rows >= 10 || s.columns > 4
                    ? (a._readabilityDataTable = !0)
                    : (a._readabilityDataTable = s.rows * s.columns > 10)
          ...
          if ("src" !== (r = e.attributes[n]).name && "srcset" !== r.name && "alt" !== r.name) {
            ...
            if (
              (/\.(jpg|jpeg|png|webp)\s+\d/.test(r.value)
                ? (l = "srcset")
                : /^\s*\S+\.(jpg|jpeg|png|webp)\S*\s*$/.test(r.value) && (l = "src"),
              l)
            )
    ...
    this._flagIsActive(this.FLAG_CLEAN_CONDITIONALLY) &&
      this._removeNodes(this._getAllNodesWithTag(e, [t]), function (e) {
        ...
        if (!a) {
          ...
          (this._forEachNode(s, (e) => (r += this._getInnerText(e).length)),
            (a = r / this._getInnerText(e).length > 0.9));
        }
        if ("table" === t && i(e)) {
          return !1;
        }
        if (this._hasAncestorTag(e, "table", -1, i)) {
          return !1;
        }
        if (this._hasAncestorTag(e, "code")) {
          return !1;
        }
        if ([...e.getElementsByTagName("table")].some((e) => e._readabilityDataTable)) {
          return !1;
        }
        ...
        if (this._getCharCount(e, ",") < 10) {
          for (
            var l = e.getElementsByTagName("p").length,
              o = e.getElementsByTagName("img").length,
              h = e.getElementsByTagName("li").length - 100,
              d = e.getElementsByTagName("input").length,
              c = this._getTextDensity(e, ["h1", "h2", "h3", "h4", "h5", "h6"]),
              g = 0,
              _ = this._getAllNodesWithTag(e, ["object", "embed", "iframe"]),
              m = 0;
            m < _.length;
            m++
          ) {
            ...
            if ("object" === _[m].tagName && this._allowedVideoRegex.test(_[m].innerHTML)) {
              return !1;
            }
            g++;
          }
          var p = this._getInnerText(e);
          if (this.REGEXPS.adWords.test(p) || this.REGEXPS.loadingWords.test(p)) {
            return !0;
          }
          var N = p.length,
          ...
          var y = (() => {
            const e = [];
            return (
              !T && o > 1 && l / o < 0.5 && e.push(`Bad p to img ratio (img=${o}, p=${l})`),
              !a && h > l && e.push(`Too many li's outside of a list. (li=${h} > p=${l})`),
              d > Math.floor(l / 3) && e.push(`Too many inputs per p. (input=${d}, p=${l})`),
              !a &&
                !T &&
                c < 0.9 &&
                N < 25 &&
                (0 === o || o > 2) &&
                f > 0 &&
                e.push(`Suspiciously short. (headingDensity=${c}, img=${o}, linkDensity=${f})`),
              !a &&
                n < 25 &&
                f > 0.2 + this._linkDensityModifier &&
                e.push(`Low weight and a little linky. (linkDensity=${f})`),
              n >= 25 &&
                f > 0.5 + this._linkDensityModifier &&
                e.push(`High weight and mostly links. (linkDensity=${f})`),
              ((1 === g && N < 75) || g > 1) &&
                e.push(`Suspicious embed. (embedCount=${g}, contentLength=${N})`),
              0 === o && 0 === b && e.push(`No useful content. (img=${o}, textDensity=${b})`),
              !!e.length && (this.log("Checks failed", e), !0)
            );
          })();
          if (a && y) {
            for (var A = 0; A < e.children.length; A++) {
              if (e.children[A].children.length > 1) {
                return y;
              }
            }
  ...
  _cleanMatchedNodes(e, t) {
    for (var i = this._getNextNode(e, !0), a = this._getNextNode(e); a && a != i; ) {
